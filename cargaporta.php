...
?>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <!-- Begin Page Content -->
                <h2>Cargar venta porta</h2>
  
<form action="procesar_ventaporta.php" method="post" id="formulario_venta-porta" enctype="multipart/form-data">
    <?php
    require 'conexion.php';
    ...
    <input type="hidden" name="producto" vale="PORTA">
</div>
</div>
    <div class="form-control bg-light border-0 small">
    <div class="col-sm-7">
<input type="hidden" name="vendedor" value="<?php echo $_SESSION['Nombre'];?>">
<label for="lineas">Linea a portar:</label>
<input type="text" class="form-control" id="lineas" name="lineas" maxlength="10">
<br>
<br>
<label for="numerodechip">Número de chip:</label>
<input type="number" class="form-control" id="numerodechip" name="numerodechip">
<br>
...
var doneTypingInterval = 100; // Intervalo de tiempo en milisegundos (0.1 segundos)

$(document).ready(function(){
    $('#lineas').keyup(function(){
        clearTimeout(typingTimer); // Limpiar el temporizador cada vez que se presiona una tecla

        if ($(this).val()) {
            typingTimer = setTimeout(buscarLinea, doneTypingInterval); // Configurar el temporizador si el campo no está vacío
        } else {
            $('#resultado_busqueda').html(""); // Mostrar mensaje si el campo está vacío
            $('#estado').val(''); // Limpiar el valor del input hidden cuando el campo está vacío
        }
    });
});

function buscarLinea() {
    console.log("Función buscarLinea() llamada.");
    var numeroLinea = $('#lineas').val(); // Obtener el número de línea del campo de entrada

    $.ajax({
        type: 'POST',
        url: 'buscar_linea.php', // Archivo PHP que maneja la búsqueda
        data: {lineas: numeroLinea}, // Enviar el dato como un objeto
        success: function(response){
            $('#resultado_busqueda').html(response); // Mostrar la respuesta en el elemento HTML
            
            // Supongamos que 'response' contiene un valor 'true' si la línea está en "Registro No Llame" y 'false' si no lo está.
            if(response === 'true') {
                $('#estado').val('REGISTRO NO LLAME'); // Asignar el valor correspondiente
            } else {
                $('#estado').val('POST VENTA'); // Asignar "POST VENTA" si no está en el registro "NO LLAME"
            }
        }
    });
}
...
<input type="hidden" id="estado" name="estadoventa">
    ...
    <label for="cliente">Condición:</label>
...
<br>
    ...
    <textarea name="comentarios" class="form-control" id="comentarios_porta" cols="150" rows="4"></textarea><br><br>
    <div class="form-group">

...
$registros_por_pagina = 30; // Número de registros a mostrar por página
...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if(!empty($_GET["estado"]) && $_GET["estado"] == 'DEVUELTAS'){
     ...
     $whereClause .= "estadoventa LIKE '%$Devueltas%' AND equipo LIKE '%RODRIGO%'";
    }
...
$ResultVendedores = mysqli_query($conexion, $QueryVendedores);
    ...
    // Obtener el turno y nombre del vendedor actual
    while ($rowTurno = $ResultTurnos->fetch_assoc()) {
        $Turno = $rowTurno['turno'];
        $Nombre = $rowTurno['nombre'];
    }
    
    // Consulta SQL para obtener vendedores únicos
    $sqlvendedores = "SELECT DISTINCT nombre FROM usuarios WHERE nombre <> ''";
    $result = $conexion->query($sqlvendedores);
    ?>
    <h5 class="modal-title" id="exampleModalLabel">Vendedor: <?php echo $Nombre . " - Turno: " . $Turno; ?></h5>
    <br>
  <?php
if ($Vendedor == 'Gabriel' || $Vendedor == 'RODRIGO NAHUEL BECERRA' || $Vendedor == 'Josefina Linares' || $Vendedor == 'Martina') {
     '<p>Cambiar de vendedor: 
    <select class="form-control" name="vendedor">';
    
    if ($result->num_rows > 0) {
        while ($rowVendedores = $result->fetch_assoc()) {
            $selected = ($rowVendedores['nombre'] == $Nombre) ? "selected" : "";
     '<option value="' . $rowVendedores['nombre'] . '" ' . $selected . '>' . $rowVendedores['nombre'] . '</option>';
        }
    }
...
?>
        ...
        <!--<input type="hidden" name="vendedor" value="<?php echo $Nombre;?>">-->
...
if ($resultado) {
    ...
    $diferenciaDias = ($ultimaFechaCambio !== null && $ultimaFechaCambio !== $fechaActual)
    ? (strtotime($fechaActual) - strtotime($ultimaFechaCambio)) / (60 * 60 * 24)
    : 0;
...
if ($resultado) {
    ...
    echo "<p>Error al consultar la base de datos: " . $conexion->error . "</p>";
              ...
              <?php }else{?>
                ...
                <?php 
if ($Vendedor == 'Martina' || $Vendedor == 'RODRIGO NAHUEL BECERRA' || $Vendedor == 'Luis Salgado') { // Use comparison operator
    echo '<td>

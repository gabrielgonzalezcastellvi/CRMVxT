                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <p><b>Fecha de carga:</b></p>
                            <input type="date" class="form-control" title="Fecha" name="fecha" placeholder="Fecha"
                                   aria-label="Fecha" aria-describedby="basic-addon4">
                        </div>
                        <div class="col-md-4 mb-3">
    ...
    <select class="form-control" title="Planes" name="planes" aria-label="Planes" aria-describedby="basic-addon4" size="1">
        ...
        ?>
                        ...
                        <p><b>Vendedor:</b></p>
                            ...
                            <?php
                            require 'conexion.php';
    $QueryVendedores = "SELECT nombre FROM usuarios WHERE equipo LIKE '%ASM%' AND bloqueado <> '1'";
...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if(!empty($_GET["estado"]) && $_GET["estado"] == 'DEVUELTAS'){
      ...
      echo  $whereClause .= "estadoventa LIKE '%$Devueltas%' AND equipo LIKE '%ASM%'";
    ...
    if(!empty($_GET["equipo"])){
      $whereClause .= "equipo = 'ASM' AND ";
    }

    // Verificar equipo
    
    $whereClause .= "equipo LIKE '%ASM%' AND ";
    ...
    $Query5 = "SELECT COUNT(linea) as TotalBusqueda FROM ventas WHERE equipo LIKE '%ASM%' ORDER BY fechacarga DESC LIMIT $registros_por_pagina OFFSET $offset";
}

$Query2 = "SELECT * FROM ventas WHERE equipo LIKE '%ASM%'";
$queryDevueltas = "SELECT COUNT(estadoventa) as devueltas FROM `ventas` WHERE estadoventa LIKE '%DEVUELTA%' AND equipo LIKE '%ASM%';";
$Query3 = "SELECT COUNT(linea) as total FROM ventas WHERE equipo LIKE '%ASM%'";
$Querytotalmes = "SELECT COUNT(linea) as total FROM ventas WHERE equipo LIKE '%ASM%' AND DATE_FORMAT(fechacarga, '%Y-%m') = DATE_FORMAT(CURDATE(), '%Y-%m')";
...
$date = '';
$date = date('Y-m-d');
$Query4 = "SELECT COUNT(*) as totalcarga FROM ventas WHERE fechacarga = '$date' AND equipo LIKE '%ASM%'";
...
?>   
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        ...
                        <?php 
if ($Date > 0) {
    echo '<h2 class="h3 mb-0 text-gray-800">Solicitudes del día: <b><a href="solicitudesasm?ventasdia=' . $date . '">' . $Date . '</a></b></h2>';
} else {
...
<?php  
if ($Devueltas > 0) {
    echo '<h2 class="h3 mb-0 text-gray-800">Solicitudes devueltas: <b><a href="solicitudesasm?estado%5B%5D=DEVUELTA">'.$Devueltas.'</a></b></h2>';
}
?>         
    ...
    .black-line {
        ...
        /* Otras propiedades de estilo que desees agregar */
            ...
            || $row['estadoventa'] == 'DEVUELTA CARGA TASA' || $row['estadoventa'] == 'DEVUELTA X VENDEDOR' || $row['estadoventa'] == 'DEVUELTA TASA'){
                $color = "orange";
            }else if($row['estadoventa'] == 'CANCELADA' || $row['estadoventa'] == 'RECHAZO ABD' || $row['estadoventa'] == 'SOLICITUD CANCELADA'|| $row['estadoventa'] == 'SCORE 80'){
                $color = "red";
...
<div class="modal fade" id="exampleModal<?php echo $row['id']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <form action="editarsoliasm" method="POST" id="formulario_soli" enctype="multipart/form-data">
      ...
      <div class="modal-body">
        ...
        <div class="tab-content" id="myTabContent<?php echo $row['id']; ?>">
          ...
          <br>
            ...
            <?php 
            $planAlmacenado = $row['verificacion'];
            ?>
...
<p>Planes: 
    ...
    <select class="form-control" name="status" onchange="updateChangeDate(this)">
        <?php
            $opciones = array("POST VENTA","RECHAZO ABD","CANCELADA","DEVUELTA","NO CUMPLE PERMANENCIA","PENDIENTE DE VERIFICACION","PENDIENTE CARGA DAMA","PENDIENTE AUDITORIA","SCORE 80","INFORMADA TASA","PENDIENTE CARGA TASA","INGRESADA TASA","EN TRANSITO","EN PROCESO DIGIP-TN","DIFERIDA CARGA TASA","DEVUELTA X VERIFICACION","DEVUELTA X AUDITORIA","DEVUELTA CARGA TASA","DIFERIDA AUDITORIA","SOLICITUD CUMPLIDA","DEVUELTA X VENDEDOR","DEVUELTA TASA","SOLICITUD CANCELADA","PENDIENTE CARGA MOVISTAR","DIFERIDA CARGA MOVISTAR","PENDIENTE CARGA IRIS","DEVUELTA CARGA MOVISTAR","INGRESADA MOVISTAR","REGISTRO NO LLAME"); // Agrega aquí todas las opciones necesarias
            foreach ($opciones as $opcion) {
...
?>
          ...
          <div class="tab-pane fade" id="informacionadicional<?php echo $row['id']; ?>" role="tabpanel" aria-labelledby="informacionadicional-tab<?php echo $row['id']; ?>">
            ...
            <p>Número de chip: <input type="number" class="form-control" name="numerochip" id="numerochip" value="<?php echo $row['numerodechip'];?>"></p>
...
$cuit = $row['cuitcliente'];
$fn = $row['fechanacimientocliente'];
$email = $row['email'];
$calle = $row['calle'];
$altura = $row['numero'];
$localidad = $row['localidad'];
$cp = $row['codigopostal'];
$numero = $row['numeroalternativo'];
$linea = $row['linea'];
$plan = $row['planes'];

// Construcción del mensaje
$mensaje = "¡¡Hola $nombre!!\n";
$mensaje .= "Me comunico desde el Area Administrativa de Movistar para dejarte en detalle el plan contratado...\n\n";
$mensaje .= "PLAN MOVISTAR MOVIL $plan:\n"; // Usa el plan específico del cliente
$mensaje .= "- ".$plan." incluidos para navegar\n";
$mensaje .= "- Llamadas y mensajes libres a todas las compañias\n";
$mensaje .= "- WhatsApp gratis.\n";
$mensaje .= "- 30gb de regalo (10gb x mes) activando TEST DRIVE\n";
$mensaje .= "- Pasa y pedi gigas gratis!\n";
$mensaje .= "- Guarda gigas que no uses para el mes siguiente\n";
$mensaje .= "- 20 gb extras por 12 meses\n";
$mensaje .= "- Descuento fijo por 1 año\n";
$mensaje .= "- Valor $3040 + IVA\n\n";
$mensaje .= "Recuerde que el dia de mañana de ".$row['verificacion']." se comunicaran desde el Area de calidad para hacer el repaso del mismo y proceder al envio de chip.\n";
$mensaje .= "¿Esa franja horaria le queda cómoda o desea modificarla?\n";
...
// URL de WhatsApp
$whatsapp_url = "https://wa.me/$linea?text=$mensaje_codificado";

// Mostrar el enlace
echo "<br><br><a href='$whatsapp_url' target='_blank'>Enviar mensaje por WhatsApp</a>";
?>
...
if ($resultado) {
    ...
    echo "<p>Error al consultar la base de datos: " . $conexion->error . "</p>";
              ...
              <?php }else{?>
                ...
                <?php 
if ($Vendedor == 'Martina' || $Vendedor == 'RODRIGO NAHUEL BECERRA' || $Vendedor == 'Luis Salgado') { 
    echo '<td>
        <a href="#" title="Eliminar" onclick="mostrarModal(event, ' . htmlspecialchars($row["id"]) . ', \'' . htmlspecialchars($row['documentocliente']) . '\', \'' . htmlspecialchars($row['linea']) . '\', \'' . htmlspecialchars($row['equipo']) . '\')">

...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   if(isset($_POST["numerosolicitud"]) || isset($_POST["numerolinea"]) || isset($_POST["dni"]) || isset($_POST["estado"])) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $Query = "SELECT * FROM ventas WHERE vendedor = '$Vendedor' GROUP BY documentocliente, linea ORDER BY id DESC LIMIT $registros_por_pagina OFFSET $offset";
    $Query5 = "SELECT COUNT(DISTINCT linea,documentocliente) as TotalBusqueda FROM ventas WHERE vendedor = '$Vendedor' ORDER BY id DESC LIMIT $registros_por_pagina OFFSET $offset";
  }
...
$Query2 = "SELECT * FROM ventas WHERE vendedor = '$Vendedor' GROUP BY documentocliente,linea ORDER BY fechacarga DESC";
$Result = mysqli_query($conexion, $Query);
$Query3 = "SELECT COUNT(DISTINCT linea,documentocliente) as total FROM ventas WHERE vendedor = '$Vendedor'";
$Result = mysqli_query($conexion, $Query);
...
$date = '';
$date = date('Y-m-d'); 
$Query4 = "SELECT COUNT(DISTINCT linea,documentocliente) as totalcarga FROM ventas WHERE fechacarga = '$date' AND vendedor = '$Vendedor'";
        ...
        <tbody>
            <?php while ($row = $Result->fetch_assoc()) { ?>
             ...
             if($row['estadoventa'] == 'DIFERIDA AUDITORIA'){
                ...
                $color = "#00aae4";
            }else if($row['estadoventa'] == 'DEVUELTA X AUDITORIA'){
                $color = "red";
            }else if($row['estadoventa'] == 'CANCELADA'){
                $color = "red";
            }else if($row['estadoventa'] == 'DEVUELTA X AUDITORIA' || $row['estadoventa'] == 'DEVUELTA' || $row['estadoventa'] == 'DEVUELTA X VERFICACION' || $row['estadoventa'] == 'DEVUELTA X AUDITORIA' 
...
<div class="modal fade" id="exampleModal<?php echo $row['id']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <?php
  if($EstadoVenta == 'DEVUELTA' || $EstadoVenta == 'DEVUELTA X VERIFICACION' || $EstadoVenta == 'DEVUELTA X AUDITORIA' || $EstadoVenta == 'DEVUELTA CARGA TASA' || $EstadoVenta == 'DEVUELTA POR VENDEDOR' || $EstadoVenta == 'DEVUELTA TASA' || $EstadoVenta == 'DEVUELTA CARGA MOVISTAR'){
?>
<form action="editarsoli2.php" method="POST" id="formulario_soli" enctype="multipart/form-data">
  ...
  <?php
    $opciones = array("POST VENTA","DEVUELTA","DEVUELTA X VERIFICACION","DEVUELTA X AUDITORIA","DEVUELTA CARGA TASA","DEVUELTA POR VENDEDOR","DEVUELTA TASA","DEVUELTA CARGA MOVISTAR"); // Agrega aquí todas las opciones necesarias

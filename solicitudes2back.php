...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $Query = "SELECT * FROM ventas WHERE vendedor = '$Vendedor' GROUP BY linea, documentocliente,producto,calle,numero,piso,dpto,entrecalles,provincia,localidad,codigopostal ORDER BY id DESC LIMIT $registros_por_pagina OFFSET $offset";
    $Query5 = "SELECT COUNT(DISTINCT linea, documentocliente,producto,calle,numero,piso,dpto,entrecalles,provincia,localidad,codigopostal) as TotalBusqueda FROM ventas WHERE vendedor = '$Vendedor' ORDER BY id DESC LIMIT $registros_por_pagina OFFSET $offset";
  }
...
$Query2 = "SELECT * FROM ventas WHERE vendedor = '$Vendedor' GROUP BY linea, documentocliente,producto,calle,numero,piso,dpto,entrecalles,provincia,localidad,codigopostal ORDER BY id DESC";
$Result = mysqli_query($conexion, $Query);
...
$Querytotalmes = "SELECT COUNT(linea) as total FROM ventas WHERE vendedor LIKE '%$Vendedor%' AND DATE_FORMAT(fechacarga, '%Y-%m') = DATE_FORMAT(CURDATE(), '%Y-%m')";
        ...
        /* Otras propiedades de estilo que desees agregar */
            ...
            <?php while ($row = $Result->fetch_assoc()) { ?>
               ...
               || $row['estadoventa'] == 'DEVUELTA CARGA TASA' || $row['estadoventa'] == 'DEVUELTA X VENDEDOR' || $row['estadoventa'] == 'DEVUELTA TASA'){
                   $color = "orange";
               }else if($row['estadoventa'] == 'CANCELADA'|| $row['estadoventa'] == 'PAGO DEUDA MOVI' || $row['estadoventa'] == 'SOLICITUD CANCELADA'){
                   $color = "red";
